...
if (isset($_SESSION['user_hash']) && isset($_SESSION['user_id']) && !empty($_SESSION['user_hash']) && !empty($_SESSION['user_id'])) {
    ...
    if (isset($_POST['handshake']) && !empty($_POST['handshake']) && isset($_POST['serverID']) && !empty($_POST['serverID'])) {
        if ($_SESSION['user_hash'] === $_POST['handshake']) {
            ...
            $SSH->exec("cd " . DIR_HOME . " && bash stop.sh --screen=BOT-" . $USER . "-" . $SERVER);
            $rep = $SSH->read();
            $SSH->disablePTY();
            $SSH->disconnect();

            if (strstr($rep, "SERVER_STOPPING")) {
                die($API->response(true, "SERVER_STOPPING"));
            } else if (strstr($rep, "SERVER_OFFLINE")) {
                die($API->response(false, "SERVER_OFFLINE"));
            }
